...
class Rick(object):
    def __init__(self,boardname,x0,y0,length,dx,dy,boards):
        ...
        self.length=length
    def move(self):
        self.tLx=int(self.tLx+self.dx/self.board.gravity)
        self.tLy=int(self.tLy+self.dy/self.board.gravity)
    def __str__(self):
        ...
        if self.tLx<=0:
            for board in self.boards:
                if self.board.leftboard==board.name:
                    info=self.board.name
                    self.board=board
                    self.tLx=self.board.portalx
                    self.tLy=self.board.portaly
                    return 'Rick of %s moved from %s board to %s board'%(self.boardname,info,self.board.name)
